...
<html>
    ...
    <body>
        <%
            ...
            ps = con.prepareStatement("SELECT * FROM contactenos where codigo=" + codigo);
            rs = ps.executeQuery();

            while (rs.next()) {
        %>
        
        <h1 class="text-center text-primary m-5">Modificar Registros Contactenos</h1>
        <div class="container d-flex justify-content-center">
            <form action="" method="post" class="form-control py-2" style="width:800px;"> 
                ...
                Nombre:
                <input type="text" name="txtNom" placeholder="*Escribe Tu Nombre Aqui" class="form-control" value="<%= rs.getString("nombre")%>"/>
                Apellido:
                <input type="text" name="txtApe" placeholder="*Escribe Tu Apellido Aqui" class="form-control"  value="<%= rs.getString("apellido")%>"/>
                Email:
                <input type="text" name="txtEmail" class="form-control" placeholder="name@ejemplo.com" value="<%= rs.getString("email")%>"/>
                Descripcion:
...
<%
    ...
    if (nom != null && apel != null && email != null && desc != null) {
        ps = con.prepareStatement("update contactenos set nombre='" + nom + "',apellido='" + apel + "',email='" + email + "',descripcion='" + desc + "'where codigo=" + codigo);
        ps.executeUpdate();
